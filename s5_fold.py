...
def s5_main(state):
    ...
    def trainres(gcn, gm, gd, A0):  # 添加gm和gd参数
        if args.cuda:
            gcn = gcn.cuda()

        train(gcn, gm, gd, A0, args.epochs, args.alpha)
        gcn.eval()
        yli, ypi = gcn(gm, gd, A0)
    ...
    def fivefoldcv_sign(A):
        ...
        for i in range(state.cv_splits):
            ...
            resi = trainres(GCNs(gm, gd), gm, gd, A0)  # 传递gm和gd参数
            if args.cuda:
